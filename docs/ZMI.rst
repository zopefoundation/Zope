...
The following example code shows a whole restructured DTML template rendering the
Zope 4 ZMI:
...
**File upload element:**
The file upload element has its own form container (classfied as ``zmi-upload``).
...
contain the ``<main>`` element are automatically post-processed by a JavaScript
