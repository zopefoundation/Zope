...
class IPubBeforeStreaming(Interface):
    ...
    something calls response.write() for the first time.

    Note that this is carries a reference to the *response*, not the
    request.
    """
...
class UseTraversalDefault(Exception):
    ...
    This exception can be raised by '__bobo_traverse__' implementations
    to indicate that it has no special casing for the given name and
    that standard traversal logic should be applied.
    """
